...
namespace IEManageSystem.Services.ManageHome.AuthorizeManage.ApiScopes
{
    public interface IApiScopeManageAppService : IApplicationService
    {
        ...
        Task<GetApiSinglesOutput> GetApiSingles(GetApiSinglesInput input);

        Task<AddApiSingleOutput> AddApiSingle(AddApiSingleInput input);

        Task<RemoveApiSingleOutput> RemoveApiSingle(RemoveApiSingleInput input);
    }
}
